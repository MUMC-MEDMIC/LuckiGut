...
# counting how many fruits and vegetables there are ====
...
mon4 <- mon4 %>% select(fruits,vegies)
mon5 <- mon5 %>% select(fruits,vegies)
mon6 <- mon6 %>% select(fruits,vegies)
mon9 <- mon9 %>% select(fruits,vegies)
mon11 <- mon11 %>% select(fruits,vegies)
mon14 <- mon14 %>% select(fruits,vegies)
...
rm(baku_f_I,food,meta,fruits,vegies)

# Kruskal Wallis test, as n count is small ====
mega$fruits_vegies <- mega$fruits+mega$vegies

kruskal <- mega %>%
  group_by(Age_individual) %>% 
  summarise(
    kruskal_test = list(kruskal.test(fruits_vegies ~ diet_class)),  # Apply Kruskal-Wallis test
    .groups = 'drop'
  ) %>%
...
#write.table (kruskal, "S3.3_kruskal_count_fruit_vegies.txt", row.names=TRUE,sep = "\t")
...
dunn <- mega  %>% 
  ...
  summarise(
    dunn_test = list(
      dunnTest(fruits_vegies ~ diet_class, method = "bh")),
    .groups = "drop"
  ) %>%
...
#write.table (dunn, "S3.3_dunn_count_fruit_vegies.txt", row.names=TRUE,sep = "\t")
...
ggplot(mega, aes(x = diet_class, y = (vegies+fruits))) +
  geom_boxplot(aes(fill = diet_class)) + 
  facet_grid( ~ Age_individual) +
  ggtitle("") +
  xlab("Diet class") +
  ylab("Fruit & vegies count")  + theme_minimal()+
  theme(legend.position = "none")

# boxplot with significance ====
plot1 <- ggplot(mon4, aes(x = diet_class, y = fruits_vegies)) +
  geom_boxplot(aes(fill = diet_class)) + 
  ggtitle("") +
  facet_grid( ~ Age_individual) +
  xlab("") +
  ylab("Count of fruits and vegitables") + theme_minimal()+
  theme(legend.position = "none",
...
plot2 <- ggplot(mon5, aes(x = diet_class, y = fruits_vegies)) +
  geom_boxplot(aes(fill = diet_class)) + 
  ggtitle("") +
  facet_grid( ~ Age_individual) +
  xlab("") +
...
plot3 <- ggplot(mon6, aes(x = diet_class, y = fruits_vegies)) +
  geom_boxplot(aes(fill = diet_class)) + 
  ggtitle("") +
  facet_grid( ~ Age_individual) +
  xlab("") +
...
plot4 <- ggplot(mon9, aes(x = diet_class, y = fruits_vegies)) +
  geom_boxplot(aes(fill = diet_class)) + 
  ggtitle("") +
  facet_grid( ~ Age_individual) +
  xlab("") +
...
plot5 <- ggplot(mon11, aes(x = diet_class, y = fruits_vegies)) +
  geom_boxplot(aes(fill = diet_class)) + 
  ggtitle("") +
  facet_grid( ~ Age_individual) +
  xlab("Diet class") +
  ...
  theme(legend.position = "none",
        ...
        panel.grid.major = element_line(color = "gray95", linewidth = 0.4),
        panel.grid.minor = element_blank())
plot5 <- plot5 + stat_pvalue_manual(dunn11,y.position = 14.5, step.increase = 0.08,tip.length = 0,
                                    label = "sig_adj",hide.ns = TRUE)

plot6 <- ggplot(mon14, aes(x = diet_class, y = fruits_vegies)) +
  geom_boxplot(aes(fill = diet_class)) + 
  ggtitle("") +
  facet_grid( ~ Age_individual) +
  xlab("") +
  ...
  theme(legend.position = "none",
        ...
        panel.grid.major = element_line(color = "gray95", linewidth = 0.4),
        panel.grid.minor = element_blank())
plot6 <- plot6 + stat_pvalue_manual(dunn14,y.position = 13, step.increase = 0.05,tip.length = 0,
                                    label = "sig_adj",hide.ns = TRUE)
