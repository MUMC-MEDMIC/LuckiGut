...
# read in data ====
spearman = read.delim("Output/Files/S4.1_baku_vs_fiber_per_TP_spearman_adjust.txt",sep = "\t",header = TRUE, na.strings=c("","NA"))
...
#write.table (results_KW_4mon, "S4.6_fiber_vs_diet_results_KW_4months.txt", row.names=FALSE,sep = "\t")
...
#write.table (results_dunn_mon4, "S4.6_fiber_vs_diet_results_dunn_4months.txt", row.names=FALSE,sep = "\t")
...
plot_mon4_rhamnogal <- ggplot(mon4_rhamnogal, aes(x = diet_class, y = Relative_abundance)) +
  ...
  scale_fill_manual(values = species_colors_5) +
...
plot_mon4_rhamnogal <- plot_mon4_rhamnogal + stat_pvalue_manual(sig_rha, label = "sig_p_adj",y.position = 3.3, step.increase = 0.001,tip.length = 0)
...
plot_mon4_arabinoxylan_with <-  plot_mon4_arabinoxylan_with + stat_pvalue_manual(sig_arab, label = "sig_p_adj",y.position = 4.4, step.increase = 0.001,tip.length = 0)
...
plot_mon4_xylan <- ggplot(mon4_xylan, aes(x = diet_class, y = Relative_abundance)) +
  ...
  theme(legend.position = "none",
        ...
        panel.grid.minor = element_blank(),
        plot.title = element_text(size = 6))
plot_mon4_xylan <- plot_mon4_xylan + stat_pvalue_manual(sig_xyl, label = "sig_p_adj",y.position = 3.3, step.increase = 0.05,tip.length = 0)
...
plot_mon4_arabinoxylan_without <- ggplot(mon4_arabinoxylan, aes(x = diet_class, y = Relative_abundance)) +
  geom_bar(aes(fill = Species_groupped),stat = "identity", position = "stack") +
  scale_fill_manual(values = species_colors_14) +
  labs(x = "Diet Class", y = "Relative abundance")  + ggtitle("Arabinoxylan at 4 months")+
  theme_minimal()+
  ...
  guides(fill = guide_legend(override.aes = list(shape = 22, size = 1)))
plot_mon4_arabinoxylan_without <-  plot_mon4_arabinoxylan_without + stat_pvalue_manual(sig_arab, label = "sig_p_adj",y.position = 4.4, step.increase = 0.001,tip.length = 0)

barplot_without <- (plot_mon4_rhamnogal + plot_mon4_xylan) / (plot_mon4_arabinoxylan_without +plot_spacer())+
  plot_layout() & 
  theme(plot.margin = margin(1, 0, 2, 0)) #top, right, bottom, left
barplot_without
...
plot_mon4_rhamnogal <- plot_mon4_rhamnogal + stat_pvalue_manual(sig_rha, label = "sig_p_adj",y.position = 3.3, step.increase = 0.001,tip.length = 0,size = 10)

plot_mon4_arabinoxylan_with <- ggplot(mon4_arabinoxylan, aes(x = diet_class, y = Relative_abundance)) +
  geom_bar(aes(fill = Species_groupped),stat = "identity", position = "stack") +
  scale_fill_manual(values = species_colors_14) +
  labs(x = "Diet Class", y = NULL)  + ggtitle("Arabinoxylan at 4 months")+
  theme_minimal()+
  labs(fill = "Degraders")+
...
plot_mon4_arabinoxylan_with <-  plot_mon4_arabinoxylan_with + stat_pvalue_manual(sig_arab, label = "sig_p_adj",y.position = 4.4, step.increase = 0.001,tip.length = 0,size = 10)
...
plot_mon4_xylan <- ggplot(mon4_xylan, aes(x = diet_class, y = Relative_abundance)) +
  ...
  theme(legend.position = "none",
        ...
        panel.grid.minor = element_blank(),
        plot.title = element_text(size = 15))
plot_mon4_xylan <- plot_mon4_xylan + stat_pvalue_manual(sig_xyl, label = "sig_p_adj",y.position = 3.3, step.increase = 0.05,tip.length = 0,size = 10)
...
ggsave(
  filename   = "S4.6_ifdp_degraders_abundance_per_fiber_with.svg",
  plot       = barplot_with,
  width    = 80,
  height   = 90,
  units    = "mm",
...
ggsave(
  filename   = "S4.6_ifdp_degraders_abundance_per_fiber_without.svg",
  plot       = barplot_without,
  width    = 80,
  height   = 90,
  units    = "mm",
  device   = svglite
