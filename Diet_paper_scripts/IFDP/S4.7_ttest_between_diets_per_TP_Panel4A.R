...
heatmap_without <- Heatmap(wide_stat, cluster_columns = T, cluster_rows = F,
                        cell_fun = function(j, i, x, y, width, height, fill) {
                          grid.text(wide_sig[i,j], x, y, gp = gpar(fontsize = 6))
                        },
                        row_labels = gsub("c", "C", gsub("\\..*", "", rownames(wide_stat))),
                        ...
                        right_annotation = rowAnnotation(Age = anno_block(gp = gpar(fill = c("#BFD200","#AACC00", "#80B918",  "#55A630", "#2B9348" ,"#007F5F"), alpha = 0.7),
                                                                          ...
                                                                          labels_gp = gpar(col = "black", fontsize = 6),
                                                                          show_name = T,
                                                                          labels_rot = 0)),show_heatmap_legend = FALSE)
heatmap_without 
...
svglite("S4.7_heatmap_without.svg",
        width  = w_cm / 2.54,
        height = h_cm / 2.54)

draw(heatmap_without)  # important for ComplexHeatmap
dev.off()
