...
# Import data ====
model <- read.delim("Output/Files/S5.13_PERMANOVA_results_without_siblings_raw.txt",sep = "\t",header = TRUE, na.strings=c("","NA"))
...
model$p_adjusted <- p.adjust(model$`Pr..F.`, method = "fdr")
...
model <- model %>% 
  mutate(sig_adj = if_else(
    ...
    true = NA, 
    false = if_else(
      condition = .$Pr..F. < 0.001,
      true = "***",
      false = if_else(
        condition = .$Pr..F. <= 0.05&.$Pr..F. >= 0.01,
        true = "*",
        false = "**"))))
...
# saving data ====
#write.table (model, "S5.14_PERMANOVA_results_cleaned_without_siblings.txt", row.names=FALSE,sep = "\t")
