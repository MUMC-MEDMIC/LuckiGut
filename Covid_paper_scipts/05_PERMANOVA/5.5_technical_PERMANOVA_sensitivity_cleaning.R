...
# Import data ====
model <- read.delim("Output/Files/S5.4_tech_PERMANOVA_results_raw.txt",sep = "\t",header = TRUE, na.strings=c("","NA"))
...
model$variables[grepl("bioinfo_batch",model$variables)]<-"Sequencing batch"
...
model <- model %>%
  mutate(Exposure_categories = if_else(
    condition = .$variables == "Sequencing batch",
    true = "Technical variable", 
    false = Exposure_categories))
...
# saving data ====
#write.table (model, "S5.5_tech_PERMANOVA_results_cleaned.txt", row.names=FALSE,sep = "\t")
