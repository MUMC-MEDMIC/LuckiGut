...
# Import data ====
model <- read.delim("Output/Files/S5.19_PERMANOVA_samples_time_spend_in_pandemic_continuous_results_dichotomized_raw.txt",sep = "\t",header = TRUE, na.strings=c("","NA"))
...
model$variables[grepl("days_group",model$variables)]<-"Time spent in pandemic"
...
model <- model %>%
  mutate(Exposure_categories = if_else(
    condition = .$variables == "Daycare"|.$variables == "Pets"|.$variables == "Pets pregnancy"|
      .$variables == "Older siblings"|.$variables == "Siblings in cohort"|.$variables == "Time spent in pandemic",
    true = "Environmental", 
    false = Exposure_categories))
...
# saving data ====
#write.table (model, "S5.20_PERMANOVA_results_time_spent_in_pandemic_dichotomized_cleaned.txt", row.names=FALSE,sep = "\t")
