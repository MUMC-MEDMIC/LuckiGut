...
# Import data ====
model <- read.delim("Output/Files/S5.7_PERMANOVA_results_index_raw.txt",sep = "\t",header = TRUE, na.strings=c("","NA"))
...
model$variables[grepl("index",model$variables)]<-"Exposure index"
...
model <- model %>%
  mutate(Exposure_categories = if_else(
    condition = .$variables == "Daycare"|.$variables == "Pets"|.$variables == "Pets pregnancy"|.$variables == "Exposure index"|
...
# saving data ====
#write.table (model, "S5.8_index_PERMANOVA_results_cleaned.txt", row.names=FALSE,sep = "\t")
