...
# Import data ====
model <- read.delim("Output/Files/S5.20_PERMANOVA_results_time_spent_in_pandemic_dichotomized_cleaned.txt",sep = "\t",header = TRUE, na.strings=c("","NA"))
...
# saving data ====
#write.table (final, "S5.20_PERMANOVA_final_samples_in_pandemic.txt", row.names=FALSE,sep = "\t")
...
plot <- ggplot(data=final, aes(x=variables, y=R2,fill=Exposure_categories)) +
  ...
  scale_fill_manual(values = c("Dietary" = "orange",
                               ...
                               "Parental"="#0072B2",
                               "Perinatal"="darkgreen",
                               "Infant's health" = "#CC79A7"))+theme_bw()+
  labs(fill = "Exposure \ncategories")+
...
ggsave(
  filename   = "S5.21_PERMANOVA_final_dichotomized.tiff",
  plot       = plot,
