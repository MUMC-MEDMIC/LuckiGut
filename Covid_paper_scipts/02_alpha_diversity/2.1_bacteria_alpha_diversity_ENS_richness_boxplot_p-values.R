...
stat_test$Age_individual<- factor(stat_test$Age_individual, levels = c("1-2 weeks", "4 weeks", "8 weeks","4 months", "5 months", "6 months", "9 months", "11 months", "14 months"))
...
wil_1w_4w <- stat_test %>%filter(Age_individual == "1-2 weeks" | Age_individual == "4 weeks") %>% subset(duplicated(Family_ID) | duplicated(Family_ID, fromLast=TRUE))
wil_4w_8w <- stat_test %>%filter(Age_individual == "4 weeks" | Age_individual == "8 weeks") %>% subset(duplicated(Family_ID) | duplicated(Family_ID, fromLast=TRUE))
wil_8w_4m <- stat_test %>%filter(Age_individual == "8 weeks" | Age_individual == "4 months") %>% subset(duplicated(Family_ID) | duplicated(Family_ID, fromLast=TRUE))
wil_4m_5m <- stat_test %>%filter(Age_individual == "4 months" | Age_individual == "5 months") %>% subset(duplicated(Family_ID) | duplicated(Family_ID, fromLast=TRUE))
wil_5m_6m <- stat_test %>%filter(Age_individual == "5 months" | Age_individual == "6 months") %>% subset(duplicated(Family_ID) | duplicated(Family_ID, fromLast=TRUE))
wil_6m_9m <- stat_test %>%filter(Age_individual == "6 months" | Age_individual == "9 months") %>% subset(duplicated(Family_ID) | duplicated(Family_ID, fromLast=TRUE))
wil_9m_11m <- stat_test %>%filter(Age_individual == "9 months" | Age_individual == "11 months") %>% subset(duplicated(Family_ID) | duplicated(Family_ID, fromLast=TRUE))
wil_11m_14m <- stat_test %>%filter(Age_individual == "11 months" | Age_individual == "14 months") %>% subset(duplicated(Family_ID) | duplicated(Family_ID, fromLast=TRUE))
...
plot_infant <- stat_test %>% ggplot(aes(y = ENS, x = Age_individual, color = Age_individual)) +
  ...
  xlab("")+ylab("")+theme(legend.position = "none",
                          ...
                          axis.text.y = element_text(size = 15))+scale_color_viridis(discrete = TRUE)
...
boxplot_rich <- plot_infant + stat_pvalue_manual(wilcox_rich,
                                                    y.position = 60, step.increase = 0.1,size = 8,
                                                    label = "p_sig_test_adj",hide.ns = TRUE)
boxplot_rich

# saving figures for the paper ====
ggsave("S2.1_richness_barplot_stats.tiff", boxplot_rich, width = 400, height = 300, dpi = 300, units = "mm")
#ggsave("S2.1_richness_barplot_stats.pdf", boxplot_rich, device = "pdf",width = 400, height = 300, dpi = 300, units = "mm")

ggsave("S2.1_ENS_barplot_stats.tiff", boxplot_ENS, width = 400, height = 300, dpi = 300, units = "mm")
#ggsave("S2.1_ENS_barplot_stats.pdf", boxplot_ENS, device = "pdf",width = 400, height = 300, dpi = 300, units = "mm")
