...
# Import data ====
model_without <- read.delim("Output/Files/S6.18_LR_pandemic_ENS_scaled_cleaned_without_outliers.txt",sep = "\t",header = TRUE, na.strings=c("","NA"))
model_with <- read.delim("Output/Files/S6.2_LR_pandemic_ENS_scaled_cleaned.txt",sep = "\t",header = TRUE, na.strings=c("","NA"))
...
plot <- ggplot(data=final, aes(x=variables, y=Estimate,fill=Exposure_categories)) +
  ...
  geom_text(aes(label = sig_padj), colour = "black",size = 4) + ggtitle("ENS")+ xlab("")+
...
ggsave("S6.19_LR_pandemic_ENS_scaled.tiff", plot, width = 400, height = 300, dpi = 300, units = "mm")
#ggsave("S6.19_LR_pandemic_ENS_scaled.pdf", plot, device = "pdf",width = 400, height = 300, dpi = 300, units = "mm")
