...
# removing outlier 
meta_age <- meta_age %>% filter(MMHP_SampleID != "MMHP_UM_LCK_249") # found in scipt S6.8
meta_age <- meta_age %>% select (all_of(c("observed_richness", variables)))
...
# removing outlier 
meta_age <- meta_age %>% filter(MMHP_SampleID != "MMHP_UM_LCK_189")# found in scipt S6.8
meta_age <- meta_age %>% select (all_of(c("observed_richness", variables)))
...
# saving data ====
#write.table (all, "S6.21_LR_pandemic_rich_scaled_raw_without_outliers.txt", row.names=FALSE,sep = "\t")
...
# saving data ====
#write.table (final_diagnostics, "S6.21_LR_pandemic_rich_scaled_residuals_raw_without_outliers.txt", row.names=TRUE,sep = "\t")
