...
# Import data ====
model <- read.delim("Output/Files/S6.10_LR_index_ENS_scaled_cleaned.txt",sep = "\t",header = TRUE, na.strings=c("","NA"))
...
# Barplot final model ====
plot <- ggplot(data=filtered_data, aes(x=variables, y=Estimate,fill=Exposure_categories)) +
...
#ggsave("S6.11_LR_index_ENS_scaled.pdf", plot, device = "pdf",width = 400, height = 300, dpi = 300, units = "mm")

ggsave(
  filename   = "S6.11_LR_index_ENS_scaled.tiff",
  plot       = plot,
