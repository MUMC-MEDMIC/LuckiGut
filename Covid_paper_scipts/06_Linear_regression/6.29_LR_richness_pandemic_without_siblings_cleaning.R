...
# Import data ====
model <- read.delim("Output/Files/S6.28_LR_pandemic_rich_scaled_raw_without_siblings.txt",sep = "\t",header = TRUE, na.strings=c("","NA"))
...
model$p_adjusted <- p.adjust(model$p.value, method = "fdr")
...
# saving data ====
#write.table (model, "S6.29_LR_pandemic_rich_scaled_without_siblings_cleaned.txt", row.names=FALSE,sep = "\t")
