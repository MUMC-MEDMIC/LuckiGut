...
# Import data ====
model <- read.delim("Output/Files/S6.29_LR_pandemic_rich_scaled_without_siblings_cleaned.txt",sep = "\t",header = TRUE, na.strings=c("","NA"))
...
final <- model %>% filter(Term != "(Intercept)")
...
ggsave(
  filename   = "S6.30_LR_richness_scaled_without_siblings.tiff",
  plot       = plot,
