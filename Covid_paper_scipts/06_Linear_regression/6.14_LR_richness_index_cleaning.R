...
# Import data ====
model <- read.delim("Output/Files/S6.13_LR_rich_index_scaled_raw.txt",sep = "\t",header = TRUE, na.strings=c("","NA"))
...
model <- model %>%
  mutate(Exposure_categories = if_else(
    condition = .$variables == "Daycare (ref: no)"|.$variables == "Pets (ref: no)"|.$variables == "Pets pregnancy (ref: no)"|
      .$variables == "Older siblings (ref: no)"|.$variables == "Siblings in cohort (ref: no)"|.$variables == "Exposure index",
    true = "Environmental", 
    false = Exposure_categories))
...
# saving data ====
#write.table (model, "S6.14_LR_index_rich_scaled_cleaned.txt", row.names=FALSE,sep = "\t")
