...
permanova = read.delim("Output/Files/S5.19_PERMANOVA_samples_time_spend_in_pandemic_continuous_results_dichotomized_raw.txt",sep = "\t",header = TRUE, na.strings=c("","NA"))
...
meta <- as.data.frame(as.matrix(infant_time@sam_data)) 
...
variables14 <- variables[variables != "twins.siblings_in_lucki"]
variables14 <- variables14[variables14 != "smoke_before_preg"]
meta_age <- meta_age %>% select (all_of(c("observed_richness", variables14)))
...
while (!stop_condition) {
  # Create the formula dynamically
  formula <- as.formula(paste("observed_richness ~", paste(variables14, collapse = "+")))
  ...
  if (max(p_values_df$p.value[-1]) <= 0.2) {
    ...
    # Find the variable with the highest p-value (excluding intercept)
    highest_pvalue_index <- which.max(p_values_df$p.value[-1])
    variable_to_remove <- variables14[highest_pvalue_index]
    
    # Remove variable with highest p-value
    variables14 <- variables14[-highest_pvalue_index]
  }
  
  # Stop if all variables have been removed
  if (length(variables14) == 0) {
    stop_condition <- TRUE
  }
}
...
rm(meta,permanova_run1)

# saving data ====
#write.table (all, "S6.34_LR_pandemic_rich_scaled_time_spent_in_pandemic_dichotomized_raw.txt", row.names=FALSE,sep = "\t")
...
rm(model_index_1w,model_index_4w,model_index_8w,model_index_4m,model_index_5m,model_index_6m,
   model_index_9m, model_index_14m)
...
final_diagnostics <- rbind(final_diagnostics_1w,final_diagnostics_4w,final_diagnostics_8w,
                           ...
                           final_diagnostics_9m,final_diagnostics_14m)
...
# saving data ====
#write.table (final_diagnostics, "S6.34_LR_pandemic_rich_scaled_residuals_time_spent_in_pandemic_dichotomized_raw.txt", row.names=TRUE,sep = "\t")
