...
# removing outlier 
meta_age <- meta_age %>% filter(MMHP_SampleID != "MMHP_UM_LCK_249") # found in scipt S6.4
meta_age <- meta_age %>% select (all_of(c("ENS", variables)))
...
# removing outlier 
meta_age <- meta_age %>% filter(MMHP_SampleID != "MMHP_UM_LCK_189")# found in scipt S6.4
meta_age <- meta_age %>% select (all_of(c("ENS", variables)))
...
# saving data ====
#write.table (all, "S6.17_LR_pandemic_ENS_scaled_raw_without_outliers.txt", row.names=FALSE,sep = "\t")
...
# saving data ====
#write.table (final_diagnostics, "S6.17_LR_pandemic_ENS_scaled_residuals_raw_without_outliers.txt", row.names=TRUE,sep = "\t")
