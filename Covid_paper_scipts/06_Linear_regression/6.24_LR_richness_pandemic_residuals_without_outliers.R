...
# load in data ====
model = read.delim("Output/Files/S6.21_LR_pandemic_rich_scaled_residuals_raw_without_outliers.txt",sep = "\t",header = TRUE, na.strings=c("","NA"))
...
model %>%
  ...
  labs(
    title = "Pandemic: Q-Q Plots. Richness studentized residuals",
...
model %>%
  ...
  facet_wrap(~Age_individual, scales = "free") +
  labs(title = "Pandemic: Studentized residuals vs fitted. Richness") +
  theme_minimal()
