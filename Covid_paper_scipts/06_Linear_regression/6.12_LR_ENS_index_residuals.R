...
# load in data ====
model = read.delim("Output/Files/S6.9_LR_ENS_index_scaled_residuals_raw.txt",sep = "\t",header = TRUE, na.strings=c("","NA"))
...
filtered_data %>%
  ...
  labs(
    title = "Index: Q-Q Plots. ENS studentized residuals",
...
filtered_data %>%
  ...
  facet_wrap(~Age_individual, scales = "free") +
  labs(title = "Index: Studentized residuals vs fitted. ENS") +
  theme_minimal()
