...
# Import data ====
model <- read.delim("Output/Files/S6.21_LR_pandemic_rich_scaled_raw_without_outliers.txt",sep = "\t",header = TRUE, na.strings=c("","NA"))

# rename variables ====
model$variables <- model$Term
plyr::count(model$Term)
model$variables[grepl("breastfeeding",model$variables)]<-"Breastfeeding (ref: no)"
model$variables[grepl("formula",model$variables)]<-"Formulafeeding (ref: no)"
...
model$variables[grepl("covid_270220",model$variables)]<-"Collected pandemic (ref: before)"
...
model$variables[grepl("ageintrosolids",model$variables)]<-"Compelmentary foods age (wks)"
model$variables[grepl("birthweight",model$variables)]<-"Birth weight (g)"

# add theme ====
model <- model %>%
  mutate(Exposure_categories = if_else(
    condition = .$variables == "Breastfeeding (ref: no)"|.$variables == "Formulafeeding (ref: no)"|.$variables == "Compelmentary foods age (wks)"|
...
model <- model %>%
  mutate(Exposure_categories = if_else(
    condition = .$variables == "Daycare (ref: no)"|.$variables == "Pets (ref: no)"|.$variables == "Pets pregnancy (ref: no)"|
      .$variables == "Older siblings (ref: no)"|.$variables == "Siblings in cohort (ref: no)"|.$variables == "Collected pandemic (ref: before)",
    true = "Environmental", 
    false = Exposure_categories))
...
# saving data ====
#write.table (model, "S6.22_LR_pandemic_rich_scaled_cleaned_without_outliers.txt", row.names=FALSE,sep = "\t")
