...
food_score <- food_score %>% dplyr::select(-dplyr::all_of(c("Family_ID", "Age_individual", "covid_270220")))
...
rm(milk,formula,anti,all, food_score)
...
# saving data ====
#write.table (summary, "S10.4_food_diversity_scores_median_iqr.txt", row.names=FALSE,sep = "\t")