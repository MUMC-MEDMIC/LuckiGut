...
ps_covid <- ps %>% ps_filter(covid_270220 == "no")

# calculating number of samples and sets
meta <- as.data.frame(as.matrix(ps_covid@sam_data))
rm(ps,ps_covid)

# merging meta with food scores ====
meta <- meta %>% mutate(merge = paste0(Family_ID, "@", Age_individual))
food_score <- food_score %>% filter(covid_270220 == "no") %>% dplyr::select(-dplyr::all_of(c("Family_ID", "Age_individual", "covid_270220")))
...
# saving data ====
#write.table (summary, "S10.6_food_diversity_scores_prepandemic_median_iqr.txt", row.names=FALSE,sep = "\t")