...
text(-1.3, yLabelPos, clusterNames) #change location of cluster numbers
text(xLabelPos, 1.3, timePtNames)#changes the location of the bar with "time point names"
#text(xLabelPos, 1.5, sprintf("n = %i", timePtCounts), cex=.7)#changes the location of the bar with "number of samples"
mtext("DMM clusters", side = 2, line = -5, at = 0, cex = 1.2) #in preview will be seen as overlap of x axes on th eplot, but if saved for publication looks ok
mtext("Age in days", side = 3, line = -2, at = 0, cex = 1.2)
...
#save as pdf, A4, landscape, 8.27x11.69
#dev.copy2pdf(file="H:/Penders_lab/Theoretical_work/Paper_1/Lucki_DMM_Transitions_Over_Time.pdf",width = 9.5, height = 6)
