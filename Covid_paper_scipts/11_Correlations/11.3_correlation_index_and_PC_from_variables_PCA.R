...
scores <- read.delim("Output/Files/S3.4_PCA_index_factor_analysis_scores_from_PCA.txt",sep = "\t",header = TRUE, na.strings=c("","NA"))

# cleaning data ====
index$Family_ID <- rownames(index)

scores_PC <- scores %>% select(PC1,PC2,PC3,PC4,PC5)
...
# scatter plot ====
plot <- ggplot(scat, aes(x=index_m_h, y=PC1)) + geom_point()+ labs(title = "")+
